    ...
    },
    {
      ...
      value: function emit(event, payload) {
        var _iterator = _createForOfIteratorHelper(
            _classPrivateFieldGet2(_listeners, this)
          ),
          _step;
        try {
...
var checkParams = function checkParams(config) {
  ...
  for (
    var _i = 0, _booleanProps = booleanProps;
    _i < _booleanProps.length;
    _i++
  ) {
  ...
  return _createClass(MagicGrid, [
    {
      ...
      value: function initStyles() {
        ...
        for (var i = 0; i < items.length; i++) {
          ...
          if (this.animate) {
            style.transition = "".concat(
              this.useTransform ? "transform" : "top, left",
              " 0.2s ease"
            );
          }
...
// masonrys에 document의 내용중 테그가 img인 것을 모두 가져온다
var masonrys = document.getElementsByTagName("img");

// 모든 이미지에 대해 'load' 이벤트가 발생했을 때 magicGrid.positionItems() 실행
for (let i = 0; i < masonrys.length; i++) {
  // 각 이미지에 'load' 이벤트 리스너를 추가하여,
  // 이미지가 로드되면 magicGrid.positionItems() 함수를 실행
